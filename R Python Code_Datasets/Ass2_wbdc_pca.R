...
pcaRadial_acc <- ggplot(resultsR, aes(x = sigma, y = best_accuracy)) +
  ...
  geom_point() +
  theme_minimal() +
  labs(title = "WBDC Dataset - PCA Radial Kernal",
       x = "Sigma",
       y = "Test Accuracy")

ggsave("pcaRadial_wbdc_acc.png", plot = pcaRadial_acc, width = 4, height = 3, dpi = 300)
...
pcaPoly_acc <- ggplot(resultsP, aes(x = sigma, y = best_accuracy)) +
  ...
  geom_point() +
  theme_minimal() +
  labs(title = "WBDC Dataset - PCA Polynomial Kernal",
       x = "Degree",
       y = "Test Accuracy")

ggsave("pcaPoly_wbdc_acc.png", plot = pcaPoly_acc, width = 4, height = 3, dpi = 300)

## lets see the scatterplot distribution for the polynomial (2) case

kpca_model <- kpca(~., data = data_pca, kernel = "polydot", 
                   features = 2, kpar = list(degree = 3, scale = 1, offset = 0))
...
polyScat <- ggplot(principal_components, aes(x = V1, y = V2, color = data.response)) + 
  geom_point() +
  labs(title = "Polynomial (3rd degree)",
       x = "Principal Component 1",
       y = "Principal Component 2") +
  theme_minimal()

ggsave("polyScat_wbdc.png", plot = polyScat, width = 4, height = 3, dpi = 300)
...
ggsave("linScat_wbdc.png", plot = linScat, width = 4, height = 3, dpi = 300)
